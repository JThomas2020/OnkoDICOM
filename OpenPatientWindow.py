...
from PySide6.QtWidgets import QWidget, QTreeWidget, \
    QTreeWidgetItem, QMessageBox, QHBoxLayout, QVBoxLayout, \
...
class UIOpenPatientWindow(object):
    ...
    def setup_ui(self, open_patient_window_instance):
        ...
        # Create a vertical box for containing the other elements
        #     and layouts
        ...
        # Create a horizontal box to hold the stop button and
        #     direction to the user on where to select the patient
        ...
        # Create a label to show direction on where the files
        #     will appear
        ...
        # Create a tree view list to list out all patients in the
        ...
        # Create a label to show what would happen if they
        #     select the patient
        ...
        self.open_patient_window_exit_button.resize(
            self.open_patient_window_stop_button.sizeHint().width(),
            self.open_patient_window_stop_button.sizeHint().height())
        self.open_patient_window_exit_button.setCursor(QtGui.QCursor(
        ...
        # Create a widget to house all of the actions button for
        #     open patient window
        ...
        # Set the vertical box fourth element, the tree view,
        #     to stretch out as far as possible
        self.open_patient_window_instance_vertical_box.setStretch(3, 4)
        # Stretch the treeview out as far as possible
        ...
        # Set the current stylesheet to the instance and connect
        #     it back to the caller through slot
    ...
    def retranslate_ui(self, open_patient_window_instance):
        ...
        self.open_patient_directory_input_box.setPlaceholderText(_translate(
            "OpenPatientWindowInstance",
            "Enter DICOM Files Path (For example, "
            "C:\path\\to\your\DICOM\Files)"))
        self.open_patient_directory_choose_button.setText(_translate(
            "OpenPatientWindowInstance", "Choose"))
        self.open_patient_directory_appear_prompt.setText(_translate(
            "OpenPatientWindowInstance",
            "Patient File directory shown below once file path chosen. "
            "Please select the file(s) you want to open:"))
        self.open_patient_directory_result_label.setText(
            "The selected directory(s) above will be opened in "
            "the OnkoDICOM program.")
    ...
    def scan_directory_for_patient(self):
        ...
        if self.filepath != "":
            ...
            # The choose button is disabled until the thread
            #     finishes executing
            self.open_patient_directory_choose_button.setEnabled(False)

            # Reveals the Stop Search button for the duration
            #     of the search
            ...
            # Then, create a new thread that will load the
            #     selected folder
    ...
    def on_search_complete(self, dicom_structure):
        ...
        :param dicom_structure: DICOMStructure object constructed
    ...
    def tree_item_changed(self, item, _):
        """
            ...
            If a different patient is checked, uncheck the
            previous patient.
            Inform user about missing DICOM files.
        """
        selected_patient = item
        # If the item is not top-level, bubble up to see which
        #     top-level item this item belongs to
        ...
        if len(list({'CT', 'MRI', 'PET'} & self.selected_series_types)) == 0:
            ...
            header = "DVH calculations are not available without a " \
                     "RTDOSE file."
        else:
            header = ""
    ...
    def on_loaded(self, results):
        ...
        if results[0] is True:
            ...
            self.patient_info_initialized.emit(results[1])
    ...
    def get_checked_leaves(self):
        """
        :return: A list of all QTreeWidgetItems in the QTreeWidget
        that are both leaves and checked.
        """
        checked_items = []
