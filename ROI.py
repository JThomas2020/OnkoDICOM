...
def rename_roi(rtss, roi_id, new_name):
    """
    Renames the given Region of Interest. Creates a csv file storing
    all the renamed ROIs for the given RTSTRUCT file.
...
def add_to_roi(rtss, roi_name, roi_coordinates, data_set):
    ...
    new_contour_number = len(rtss.ROIContourSequence[position].
                             ContourSequence) + 1
...
def get_raw_contour_data(rtss):
    ...
    :return: dict_ROI, a dictionary of ROI contours; dict_NumPoints,
...
def calculate_matrix(img_ds):
    ...
    # https://dicom.innolitics.com/ciods/rt-structure-set/roi-contour/
    #     30060039/30060040/30060050
    matrix_M = np.ndarray(
        shape=(4, 4),
        buffer=np.array(
            [
    ...
    for i in range(0, img_ds.Columns):
        i_mat = np.matmul(
            matrix_M,
            np.ndarray(
                shape=(4, 1), buffer=np.array([[i], [0], [0], [1]],
                                              dtype=float)
            ),
        )
    ...
    for j in range(0, img_ds.Rows):
        j_mat = np.matmul(
            matrix_M,
            np.ndarray(
                shape=(4, 1), buffer=np.array([[0], [j], [0], [1]],
                                              dtype=float)
            ),
        )
...
def pixel_to_rcs(pixlut, x, y):
    ...
    :param y: Pixel Y value (greater than 0, less than the slice's
    Rows data element)
    :return: The pixel coordinate converted to an RCS point as set
    by the image slice.
    """
...
def get_contour_pixel(
    ...
    for roi in roi_selected:
        # Using this type of dict to handle multiple contours within
        #     one slice
...
def create_initial_rtss_from_ct(img_ds:pydicom.dataset.Dataset,
    ...
    """
    Pre-populate an RT Structure Set based on a single CT (or MR)
    and a list of image UIDs.
    The caller should update the Structure Set Label, Name, and
    Description, which are set to "OnkoDICOM" plus the StudyID from
    the CT, and must add
    Structure Set ROI Sequence, ROI Contour Sequence, and RT ROI
    ...
    # Best to modify the Structure Set Lable with something more
    #     interesting in the application.
    ...
    # acceptable to copy because the contents of each reference
    # sequence item only include SOP Class and SOP Instance, and not
    # additional elements that are in referenced image seq but not
    #     referenced instance seq
    referenced_series_sequence_item.ReferencedInstanceSequence = \
        shallowcopy(referenced_image_sequence)
