...
def test_first_time_welcome_window(qtbot, tmpdir,
                                   init_first_time_window_config):
    first_time_welcome_window = FirstTimeWelcomeWindow()
    first_time_welcome_window.show()
    qtbot.addWidget(first_time_welcome_window)
    db_file_path = os.environ['USER_HIDDEN'] + 'TestFirstTimeWelcomeWindow.db'

    assert first_time_welcome_window.first_time_welcome_message_label.text() \
           == "Welcome to OnkoDICOM!"
    assert first_time_welcome_window.first_time_welcome_message_slogan.text() \
           == "OnkoDICOM - the solution for producing data for analysis " \
              "from your oncology plans and scans."
    assert first_time_welcome_window.first_time_welcome_default_dir_prompt.\
               text() == "Choose the path of the default directory " \
                         "containing all DICOM files:"
    assert first_time_welcome_window.first_time_welcome_input_box.\
               placeholderText() == "Enter DICOM Files Path (For example, " \
                                    "C:\path\\to\your\DICOM\Files)"
    assert first_time_welcome_window.first_time_welcome_choose_button.text() \
           == "Choose"
    ...
    qtbot.mouseClick(first_time_welcome_window.save_dir_button,
                     QtCore.Qt.LeftButton)

    # Test with directory box filled
    qtbot.keyClicks(first_time_welcome_window.first_time_welcome_input_box,
                    str(tmpdir))
    with qtbot.waitSignal(first_time_welcome_window.go_next_window,
                          raising=True):
        qtbot.mouseClick(first_time_welcome_window.save_dir_button,
                         QtCore.Qt.LeftButton)
