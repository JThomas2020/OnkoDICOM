...
class ImageLoader(QtCore.QObject):
    """
    This class is responsible for initializing and creating all the
    values required to create an instance of
    the PatientDictContainer, that is used to store all the
    DICOM-related data used to create the patient window.
    """
    ...
    def load(self, interrupt_flag, progress_callback):
        """
        :param interrupt_flag: A threading.Event() object that tells
        the function to stop loading.
        :param progress_callback: A signal that receives the current
        progress of the loading.
        :return: PatientDictContainer object containing all values
        related to the loaded DICOM files.
        """
        ...
        #     check after every step whether or not the interrupt flag has
        #     been set, in which case it will interrupt this method after
        #     the currently processing function has finished running.
        # It's not very pretty, and the thread will still run some
        #     functions for, in some cases, up to a couple seconds after
        #     the close button on the Progress Window has been clicked,
        #     however it's the best solution I could come up with.
        # If you have a cleaner alternative, please make your
        #     contribution.
        ...
        if 'rtss' in file_names_dict:
            ...
            if 'rtdose' in file_names_dict and self.calc_dvh:
                ...
                # Spawn-based platforms (i.e Windows and MacOS) have a
                #     large overhead when creating a new process, which
                #     ends up making multiprocessing on these platforms
                #     more expensive than linear calculation.
                # As such, multiprocessing is only available on Linux
