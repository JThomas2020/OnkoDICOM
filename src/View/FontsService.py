...
class FontService:
	...
	def get_scaled_font_pixel_size(self, main_app, original_font_size):
		if type(main_app) != QtWidgets.QApplication:
			...
			return None
		font_pixel_size = original_font_size * main_app.primaryScreen().\
			physicalDotsPerInch()
		current_os = platform.system()
		if current_os == 'Windows':
			font_pixel_size /= 96
		# Font on Windows should be displayed around 96 DPI
		elif current_os == 'Linux':
			font_pixel_size /= 96
		# To be decided which DPI is most suited on Linux system
		elif current_os == 'Darwin':
			font_pixel_size /= 72
		# Font on MacOS should be displayed around 72 DPI
