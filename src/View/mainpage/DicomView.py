...
class DicomView(QtWidgets.QWidget):
    ...
    def init_metadata(self):
        ...
        # QGridLayout with QSpaceItems to anchor the labels to the
        # corners of the DICOM view. This caused a reintroduction of the
        # tedious memory issues that were fixed with the restructure.
        # The following was rewritten to not use QSpaceItems because
        # they, for reasons unknown, caused a memory leak resulting in
        # the entire patient dictionary not being cleared from memory
        # correctly, leaving hundreds of additional megabytes unused in
        # memory each time a new patient was opened.
    ...
    def update_view(self, zoom_change=False):
        ...
        if zoom_change:
            self.view.setTransform(QtGui.QTransform().scale(self.zoom,
                                                            self.zoom))
    ...
    def roi_display(self):
        ...
        """
        slider_id = self.slider.value()
        curr_slice = self.patient_dict_container.get(
            "dict_uid")[slider_id]

        selected_rois = self.patient_dict_container.get(
            "selected_rois")
    ...
    def calc_roi_polygon(self, curr_roi, curr_slice, dict_rois_contours):
        ...
        # determine if it is contained entirely within the  largest
        # contour.
        ...
        # polygon from the largest polygon, and then remove the polygon
        # from the list of polygons to be displayed. This process should
        # provide fast and reliable results, however it should be noted
        # that this method may fall apart in a situation where there are
        # multiple "large" polygons, each with their own hole in it. An
        # appropriate solution to that may be to compare every contour
        # against one another and determine which ones have holes
        # encompassed entirely by them, and then subtract each hole
        # from the larger polygon and delete the smaller holes. This
        # second solution would definitely lead to more accurate
        # representation of contours, but could possibly be too slow to
        # be viable.
    ...
    def calc_dose_polygon(self, dose_pixluts, contours):
        ...
        for contour in contours:
            list_qpoints = []
            # Slicing controls how many points considered for
            # visualization
