...
class UIMainWindow:
    """
    The central class responsible for initializing most of the values
    stored in the PatientDictContainer model and defining the visual
    layout of the main window of OnkoDICOM. No class has access to the
    attributes belonging to this class, except for the class's
    ActionHandler, which is used to trigger actions within the main
    window. Components of this class (i.e. QWidget child classes such as
    StructureTab, DicomView, DicomTree, etc.) should not be able to
    reference this class, and rather should exist independently and only
    be able to communicate with the PatientDictContainer model. If a
    component needs to communicate with another component, that should
    be accomplished by emitting signals within that components, and
    having the slots for those signals within this class (as
    demonstrated by the update_views() method of this class). If a class
    needs to trigger one of the actions defined in the ActionHandler,
    then the instance of the ActionHandler itself can safely be passed
    into the class.
    """
    ...
    def setup_central_widget(self):
        ...
        # Left panel contains stuctures tab, isodoses tab, and structure
        # information
        ...
        if not patient_dict_container.has_modality("rtss") and \
                not patient_dict_container.has_modality("rtdose"):
        ...
        if patient_dict_container.has_modality("rtss") and \
                patient_dict_container.has_modality("rtdose"):
            self.dvh_tab = DVHTab()
        ...
        # components, i.e. the Clinical Data should be a child of
        # QWidget that can exist independently of OnkoDICOM. This class
        # would differ from most other main window components in that
        ...
        # exist outside OnkoDICOM). There are two classes: one for
        # displaying the clinical data, and another for modifying the
        # clinical data. The check below determines whether there
        # already exists a clinical data csv for the patient, and loads
        # either the data display or the data form depending on what
        # exists.
    ...
    def update_views(self):
        """
        This function is a slot for signals to request the updating of
        the DICOM View and DVH tabs in order to reflect changes made by
        other components of the main window (for example, when a
        structure in the structures tab is selected, this method needs
        to be called in order for the DICOM view window to be updated to
        show the new region of interest.
        """
