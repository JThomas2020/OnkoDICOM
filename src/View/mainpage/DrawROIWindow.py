...
from PySide6.QtWidgets import QMessageBox, QHBoxLayout, QLineEdit, \
    ...
    QListWidget, QGraphicsPixmapItem, \
...
class UIDrawROIWindow:
    ...
    def setup_ui(self, draw_roi_window_instance, rois, dataset_rtss,
        ...
        # This will contain the new pixel coordinates specified by the
    ...
    def retranslate_ui(self, draw_roi_window_instance):
        ...
        draw_roi_window_instance.setWindowTitle(_translate(
            "DrawRoiWindowInstance", "OnkoDICOM - Draw Region Of Interest"))
        self.roi_name_label.setText(_translate(
            "ROINameLabel", "Region of Interest: "))
        self.roi_name_line_edit.setText(_translate("ROINameLineEdit", ""))
        self.image_slice_number_label.setText(_translate(
            "ImageSliceNumberLabel", "Image Slice Number: "))
        self.image_slice_number_line_edit.setText(_translate(
            "ImageSliceNumberLineEdit",
            str(self.dicom_view.current_slice_number)))
        self.image_slice_number_transect_button.setText(_translate(
            "ImageSliceNumberTransectButton", "Transect"))
        self.image_slice_number_box_draw_button.setText(_translate(
            "ImageSliceNumberBoxDrawButton", "Set Bounds"))
        self.image_slice_number_draw_button.setText(_translate(
            "ImageSliceNumberDrawButton", "Draw"))
        self.image_slice_number_move_forward_button.setText(_translate(
            "ImageSliceNumberMoveForwardButton", "Forward"))
        self.image_slice_number_move_backward_button.setText(
            _translate("ImageSliceNumberMoveBackwardButton", "Backward"))
        self.draw_roi_window_instance_save_button.setText(_translate(
            "DrawRoiWindowInstanceSaveButton", "Save"))
        self.draw_roi_window_instance_cancel_button.setText(_translate(
            "DrawRoiWindowInstanceCancelButton", "Cancel"))
        self.internal_hole_max_label.setText(_translate(
            "InternalHoleLabel", "Maximum internal hole size (pixels): "))
        self.internal_hole_max_line_edit.setText(_translate(
            "InternalHoleInput", "9"))
        self.isthmus_width_max_label.setText(_translate(
            "IsthmusWidthLabel", "Maximum isthmus width size (pixels): "))
        self.isthmus_width_max_line_edit.setText(_translate(
            "IsthmusWidthInput", "5"))
        self.min_pixel_density_label.setText(_translate(
            "MinPixelDensityLabel", "Minimum density (pixels): "))
        self.min_pixel_density_line_edit.setText(_translate(
            "MinPixelDensityInput", ""))
        self.max_pixel_density_label.setText(_translate(
            "MaxPixelDensityLabel", "Maximum density (pixels): "))
        self.max_pixel_density_line_edit.setText(_translate(
            "MaxPixelDensityInput", ""))
        self.draw_roi_window_viewport_zoom_label.setText(_translate(
            "DrawRoiWindowViewportZoomLabel", "Zoom: "))
    ...
    def init_layout(self):
        ...
        self.draw_roi_window_instance_action_reset_button.setObjectName(
        ...
        self.draw_roi_window_instance_action_reset_button.resize(
            ...
            self.draw_roi_window_instance_action_reset_button
                .sizeHint().height())
        self.draw_roi_window_instance_action_reset_button.clicked.connect(
            self.on_reset_clicked)
        self.draw_roi_window_instance_action_reset_button.setProperty(
        ...
        self.draw_roi_window_instance_action_reset_button.setIcon(
            icon_clear_roi_draw)
        self.draw_roi_window_input_container_box.addRow(
            self.draw_roi_window_instance_action_reset_button)
        ...
        # Create a horizontal box for containing the input fields and
        # the viewport
    ...
    def on_draw_clicked(self):
        ...
        if self.min_pixel_density_line_edit.text() == "" or \
                self.max_pixel_density_line_edit.text() == "":
            ...
            if min_pixel.isdecimal() and max_pixel.isdecimal():
                ...
                if min_pixel >= max_pixel:
                    QMessageBox.about(self.draw_roi_window_instance,
                                      "Incorrect Input",
                                      "Please ensure maximum density is at "
                                      "least higher than minimum density.")
    ...
    def on_save_clicked(self):
        ...
        if self.ds is not None:
            ...
            # form a polygon, the list of pixel points need
            ...
            if pixel_hull is not None:
                ...
                # single-dimensional array, as RTSTRUCT contour data is
                ...
                # Create a popup window that modifies the RTSTRUCT and
                # tells the user that processing is happening.
                ...
                QMessageBox.about(
                    ...
                    "Selected points will generate multiple contours, which "
                    "is not currently supported. If the region you are "
                    "drawing is not meant to generate multiple contours, "
                    "please adjust your selected alpha value.")
        else:
            QMessageBox.about(self.draw_roi_window_instance, "Not Enough Data",
    ...
    def convert_hull_to_rcs(self, hull_pts):
        ...
        :param hull_pts: List of pixel coordinates ordered to form a
        polygon.
        ...
        if hasattr(self, 'drawingROI') and len(
            ...
            if list_of_points is not None:
                ...
                QMessageBox.about(
                    self.draw_roi_window_instance,
                    "Multipolygon detected",
                    "Selected points will generate multiple contours, "
                    "which is not currently supported. "
                    "If the region you are drawing is not meant to generate"
                    " multiple contours, please adjust your selected alpha "
                    "value.")
        else:
            QMessageBox.about(self.draw_roi_window_instance, "Not Enough Data",
                              "Please ensure you have drawn your ROI first.")
...
class SaveROIProgressWindow(QtWidgets.QDialog):
    """
    This class displays a window that advises the user that the RTSTRUCT
    is being modified, and then creates a new thread where the new
    RTSTRUCT is modified.
    """
    ...
    def roi_saved(self, result):
        """
        This method is called when the second thread completes
        generation of the new dataset object.
...
class SelectROIPopUp(QDialog):
    ...
    def init_standard_names(self):
        """
        Create two lists containing standard organ and standard volume
        names as set by the Add-On options.
        """
...
class Drawing(QtWidgets.QGraphicsScene):
    ...
    def __init__(self, image_to_paint, pixmap_data, min_pixel, max_pixel,
                 dataset,
                 draw_roi_window_instance):
    ...
    def _display_pixel_color(self):
        """
        Creates the initial list of pixel values within the given
        minimum and maximum densities, then displays them on the view.
        """
        if self.min_pixel <= self.max_pixel:
            data_set = self.dataset
            if hasattr(self.draw_roi_window_instance, 'bounds_box_draw'):
                min_x = int(
                    self.draw_roi_window_instance.bounds_box_draw.box
                        .rect().x())
                min_y = int(
                    self.draw_roi_window_instance.bounds_box_draw.box
                        .rect().y())
            ...
            creates a new layer over the existing image instead of 
            replacing it.
            """
    ...
    def _find_neighbor_point(self, event):
        """
        Find point around mouse position. This function is for if we
        want to choose and drag the circle
        :rtype: ((int, int)|None)
    ...
    def get_values(self):
        """
        This function gets the corresponding values of all the points in
        the drawn line from the dataset.
        """
        for i in range(512):
            for j in range(512):
    ...
    def refresh_image(self):
        """
        Convert QImage containing modified CT slice with highlighted
        pixels into a QPixmap, and then display it onto the view.
        """
    ...
    def remove_pixels_within_circle(self, clicked_x, clicked_y):
        ...
        updates the image.
        """
        # Calculate euclidean distance between each highlighted point
        # and the clicked point. If the distance is less than
        # the radius, remove it from the highlighted pixels.
    ...
    def draw_cursor(self, event_x, event_y, new_circle=False):
        ...
        :param event_x:
        QGraphicsScene event attribute: event.scenePos().x()
        :param event_y:
        QGraphicsScene event attribute: event.scenePos().y()
        :param new_circle: True when the circle object is being created
        rather than updated.
        """
    ...
    def draw_contour_preview(self, list_of_points):
        ...
        :param list_of_points: A list of points ordered to form a
        polygon.
        """
        qpoint_list = []
...
class DrawBoundingBox(QtWidgets.QGraphicsScene):
    """
    Object responsible for updating and displaying the bounds of the ROI
    draw.
    """
