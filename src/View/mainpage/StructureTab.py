...
class StructureTab(QtWidgets.QWidget):
    ...
    def init_color_roi(self):
        ...
        :return: Dictionary where the key is the ROI number and the
        value a QColor object.
        """
        roi_color = dict()
        ...
        if len(roi_contour_info) > 0:
            for item, roi_dict in roi_contour_info.items():
                # Note: the keys of roiContourInfo are "item 0",
                # "item 1", etc. As all the ROI structures are
                # identified by the ROI numbers in the whole code, we
                # get the ROI number 'roi_id' by using the member
                # 'list_roi_numbers'.
                id = item.split()[1]
    ...
    def init_roi_buttons(self):
        ...
        # self.button_roi_delete.setToolButtonStyle(
        ...
        # self.button_roi_draw.setToolButtonStyle(
        #     QtCore.Qt.ToolButtonTextUnderIcon)
    ...
    def structure_modified(self, changes):
        ...
        description_of_changes follows the format {"type_of_change":
        value_of_change}.
        Examples: {"rename": ["TOOTH", "TEETH"]} represents that the
        TOOTH structure has been renamed to TEETH.
        {"delete": ["TEETH", "MAXILLA"]} represents that the TEETH and
        MAXILLA structures have been deleted.
        {"draw": "AORTA"} represents that a new structure AORTA has been
        drawn.
        """
        ...
        # modified indicator giving the user the option to save their
        # new file.
        ...
        # dataset with the new one so that OnkoDICOM starts working off
        # this dataset rather than the original RTSS file.
        ...
        if self.patient_dict_container.has_modality("raw_dvh"):
            ...
            # Remove structures from DVH list - the only visible effect
            # of this section is the exported DVH csv
    ...
    def show_modified_indicator(self):
        ...
        # then add them back again. This ensure that the modifier
        # appears above the ROI modify buttons.
    ...
    def structure_checked(self, state, roi_id):
        """
        Function triggered when the checkbox of a structure is checked
        or unchecked.
    ...
    def save_new_rtss(self, event=None):
        rtss_directory = str(Path(self.patient_dict_container.get(
            "file_rtss")))
