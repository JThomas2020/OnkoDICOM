...
class StructureInformation(object):
    """
	Manage all functionalities related to the Structure Information
	section (bottom left of the main page).
    ...
    def get_struct_info(self):
        """
		Dictionary for all the ROI structures containing information
		about the volume, the min, max and mean doses.

		:return:
		Dictionary where:
		- the key is the id of the ROI structure
		- the value is a dictionary whose keys are volume, min, max
		and mean.
		"""
        ...
        for roi_id, _ in self.window.rois.items():
            ...
            # The volume of the ROI is greater than 0
                ...
                """
				The min dose is the last dose (in cGy) where the 
				percentage of volume receiving this dose is equal to 
				100%. The mean dose is the last dose (in cGy) where the 
				percentage of volume receiving this dose is greater than 
				50%. The max dose is the last dose (in cGy) where the 
				percentage of volume receiving this dose is greater than 
				0%.
				"""
                volume_percent = 100 * counts / dvh.volume
                # index is used as a cursor to get the min, mean and
				# max doses.
                index = 0

                # Get the min dose of the ROI
                while index < len(volume_percent) and int(
                        volume_percent.item(index)) == 100:
                    index += 1
                if index == 0:
                    struct_info['min'] = 0
                else:
                ...
                while index < len(volume_percent) and volume_percent.item(
                        index) > 50:
                    index += 1
                if index == 0:
                    struct_info['mean'] = 0
    ...
    def item_selected(self, index):
        ...
        if index == 0:
            ...
            self.volume_value.setText(_translate("MainWindow", str(
                self.list_info[struct_id]['volume'])))
            self.min_dose_value.setText(_translate("MainWindow", str(
                self.list_info[struct_id]['min'])))
            self.max_dose_value.setText(_translate("MainWindow", str(
                self.list_info[struct_id]['max'])))
            self.mean_dose_value.setText(_translate("MainWindow", str(
                self.list_info[struct_id]['mean'])))
