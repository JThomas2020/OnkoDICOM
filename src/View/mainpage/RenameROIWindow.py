...
class RenameROIWindow(QDialog):
    ...
    def __init__(self, standard_volume_names, standard_organ_names, rtss,
        ...
        # adding icon
        self.icon.addPixmap(QtGui.QPixmap(resource_path(
            "res/images/icon.ico")), QtGui.QIcon.Normal, QtGui.QIcon.Off)
    ...
    def on_text_edited(self, text):
        if text in self.standard_volume_names or \
                text in self.standard_organ_names:
            self.feedback_text.setStyleSheet("color: green")
            self.feedback_text.setText("Entered text is in standard names")
        elif text.upper() in self.standard_volume_names or \
                text.upper() in self.standard_organ_names:
    ...
    def on_roi_renamed(self, new_rtss):
        ...
        QtWidgets.QMessageBox.about(self, "Saved",
                                    "Region of interest successfully renamed!")
        self.close()
...
class RenameROIProgressWindow(QtWidgets.QDialog):
    ...
    signal_roi_renamed = QtCore.Signal(pydicom.Dataset)
    ...
    def roi_renamed(self, result):
        ...
        :param result: The resulting dataset from the ROI.rename_roi
        function.
        """
