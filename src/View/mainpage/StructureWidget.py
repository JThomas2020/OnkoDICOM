...
class StructureWidget(QtWidgets.QWidget):
    # (new_dataset, change_description)
    structure_renamed = QtCore.Signal(tuple)
    ...
    def __init__(self, roi_id, color, text, structure_tab):
        ...
        if text in structure_tab.standard_organ_names or \
                text in structure_tab.standard_volume_names:
        ...
        # Any suffix number will still be considered standard.
        for item in structure_tab.standard_volume_names:
    ...
    def roi_suggestions(self):
        """
        Get the top 3 suggestions for the selected ROI based on string
        matching with standard ROIs provided in .csv format.

        :return: two dimensional list with ROI name and string match
        percent
        ...
        # will get the top 3 matches
        suggestions = process.extract(self.text, roi_list, limit=3)
    ...
    def context_menu_event(self, event):
        ...
        menu.setStyleSheet(
            "QMenu::item::selected {background-color: #9370DB}")
        ...
        if action == rename_action:
            rename_window = RenameROIWindow(
                ...
                self.dataset_rtss, self.roi_id, self.text,
        ...
        if not self.standard_name:
            if action == suggested_action1:
                ...
                rename_window = RenameROIWindow(
                    ...
                    self.structure_tab.standard_organ_names,
                    self.dataset_rtss, self.roi_id, self.text,
                    self.structure_renamed, suggestions[1][0])
                rename_window.exec_()
                ...
                rename_window = RenameROIWindow(
                    ...
                    self.structure_tab.standard_organ_names,
                    self.dataset_rtss, self.roi_id, self.text,
                    self.structure_renamed, suggestions[2][0])
                rename_window.exec_()
