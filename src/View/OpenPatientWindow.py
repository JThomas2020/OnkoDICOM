...
class UIOpenPatientWindow(object):
    ...
    def setup_ui(self, open_patient_window_instance):
        ...
        # Create a vertical box for containing the other elements and
        # layouts
        ...
        # Create a horizontal box to hold the stop button and direction
        # to the user on where to select the patient
        ...
        # Create a widget to house all of the action buttons for open
        # patient window
        ...
        # Create an interrupt event for stopping the directory search
    ...
    def retranslate_ui(self, open_patient_window_instance):
        ...
        self.open_patient_directory_input_box.setPlaceholderText(_translate(
            "OpenPatientWindowInstance",
            "Enter DICOM Files Path "
            "(For example, C:\path\\to\your\DICOM\Files)"))
    ...
    def scan_directory_for_patient(self):
        ...
        if self.filepath != "":
            ...
            # The choose button is disabled until the thread finishes
            # executing
            self.open_patient_directory_choose_button.setEnabled(False)

            # Reveals the Stop Search button for the duration of the
            # search
    ...
    def tree_item_changed(self, item, _):
        ...
        if len(list({'CT', 'MR', 'PET'} & self.selected_series_types)) == 0:
            ...
            header = "DVH calculations are not available " \
                     "without a RTDOSE file."
        else:
            header = ""
    ...
    def get_checked_leaves(self):
        """
        :return: A list of all QTreeWidgetItems in the QTreeWidget that
        are both leaves and checked.
        """
        checked_items = []
