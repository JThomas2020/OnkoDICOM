"""
This file holds all the user input pop up dialogs used from the
software
"""
import re
...
class DialogWindowing(QDialog):
    ...
    def __init__(self, win_name, scan, upper_level, lower_level):
        super(DialogWindowing, self).__init__()

        # Passing the current values if it is an existing option or
        # empty if its a new one
        self.win_name = win_name
        self.setWindowIcon(QtGui.QIcon(resource_path(
    ...
    # This function returns the user inputs in case of an OK button
    # being pressed
    def getInputs(self):
    ...
    def accepting(self):
        ...
        if self.name.text() != '' and self.scan_text.text() != '' and \
                self.upper_level_text.text() != '' and \
                self.lower_level_text.text() != '':

            # Check validation
            if re.match(r'^([\s\d]+)$', self.upper_level_text.text()) and \
                    re.match(r'^([\s\d]+)$',
                             self.lower_level_text.text()):
                self.accept()
            ...
            # The level fields do not contain just numbers
                ...
                button_reply = QMessageBox.warning(
                    self, "Error Message",
                    "The level fields need to be numbers!",
                    QMessageBox.Ok)
...
class DialogOrgan(QDialog):
    ...
    def __init__(self, standard_name, fma_id, organ, url):
        super(DialogOrgan, self).__init__()
    ...
    def accepting(self):
        ...
        # Check that no mandatory input is empty
        if self.standard_name_header.text() != '' and \
                self.fma_id_header.text() != '' and \
                self.organ_header.text() != '':
            # Check validation
...
class DialogVolume(QDialog):
    ...
    def __init__(self, standard_name, volume_name):
        super(DialogVolume, self).__init__()
...
#                                                                             #
#   This class creates the user input dialog for when Modifying or            #
#   Adding an ROI from ISODOSE  (FUTURE FEATURE)                              #
#                                                                             #
###############################################################################

# class DialogDose(QDialog):
#
...
#
#     def getInputs(self):
#         return (self.iso_dose.text(),
#                 str('ISO' + self.iso_dose.text()), self.iso_notes.text())
#
#     def accepting(self):
...
#                     self, "Error Message",
#                     "The Isodose level should to be a number!",
#                     QMessageBox.Ok)
