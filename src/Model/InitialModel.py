...
def create_initial_model():
    ...
    PatientDictContainer model required for the operation of the main
    window. This should be called before the main window's components
    are constructed, but after the initial values of the
    PatientDictContainer instance are set (i.e. dataset and filepaths).
    """
    ##############################
    ...
    if patient_dict_container.has_modality("rtplan"):
        # The TargetPrescriptionDose is type 3 (optional), so it may not
        # be there.
        ...
        # Additionally, even if it is TARGET, that's no guarantee that
        # TargetPrescriptionDose will be encoded and have a value.
