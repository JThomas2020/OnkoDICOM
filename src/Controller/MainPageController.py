###############################################################################
#                                                                             #
#   This file handles all the processes within the Main page window of        #
#   the software                                                              #
#                                                                             #
###############################################################################
import csv
...
###############################################################################
#                                                                             #
#   The following code are global functions and data/variables used by        #
#   both Clinical Data form and Display classes                               #
#                                                                             #
###############################################################################

# This variable holds the error messages of the Clinical data form
message = ""
...
# This function returns the difference of two dates in decimal years
...
###############################################################################
#                                                                             #
#   This Class handles the Clinical Data Form display in both new mode        #
#   and editing mode of clinical data                                         #
#                                                                             #
###############################################################################
...
class ClinicalDataForm(QtWidgets.QWidget, Ui_Form):
    ...
    # get the code for the selected disease
    def get_disease_code(self, string):
        return string[:string.index(" ")]

    ###########################################################################
    #                                                                         #
    #   The following functions handle changes on the form based on the       #
    #   selected options of some elements                                     #
    #                                                                         #
    ###########################################################################
    ...
    # Handles the change in the date of local failure according to the
    ...
    # Handles the change in date of distant failure according to the
    # option chosen in distant control.
    ...
    ###########################################################################
    #                                                                         #
    # The following function function performs some form validations          #
    # before saving and records them for display if any                       #
    #                                                                         #
    ###########################################################################
    ...
    def form_validation(self):
        ...
        if self.ui.Dt_Distant_Failure.date() > QtCore.QDate.currentDate():
            message = message + "Patient's date of distant failure cannot be " \
                                "in the future. \n "

    ###########################################################################
    #                                                                         #
    #   The following function performs the saving of the clinical data       #
    ...
    def save_clinical_data(self):
        ...
        if len(message.strip()) == 0:
            ...
            data_row = [self.pID, self.ui.gender.currentText(),
                       ...
                       self.get_disease_code(self.ui.line_histology.text(
                       )), self.get_disease_code(self.ui.line_icd.text()),
                       ...
                       self.get_code(self.ui.Chemo.currentText()), self.get_code(
            ...
            if save_reply == QtWidgets.QMessageBox.Ok:
                # When they press Okay on the pop up, display the
    ...
    ###########################################################################
    #                                                                         #
    #   The following functions are used when the form is in editing          #
    #   mode of the clinical data                                             #
    #                                                                         #
    ###########################################################################
    ...
    # get code for Surgery/Rad/Chemo/Immuno/Btrachy/Hormone
    def get_code_reverse(self, theChoice):
        if theChoice == "Pri":
            return "Primary (Pri)"
        elif theChoice == "Ref":
            return "Refused (Ref)"
    ...
    # get the disease name based on the code in the csv
    ...
    # This function alters the form UI and enters the corresponding data
    ...
    def editing_mode(self):
        ...
        # Create a dtype with the binary data format and the desired
        # column names
        ...
        self.ui.line_histology.setText(self.completer_fill(0,
                                                           clinical_data[5]))
    ...
    # The following function enables the 'Enter' keyboard button to act
    # as an activator of the 'Save' button
    def on_click(self):
...
###############################################################################
#                                                                             #
#   This Class handles the Clinical Data display of information from a        #
#   csv file                                                                  #
#                                                                             #
###############################################################################
...
class ClinicalDataDisplay(QtWidgets.QWidget, Ui_CD_Display):
    ...
    # The following function enables the 'Enter' keyboard button to act
    # as an activator of the 'Edit' button
    def on_click(self):
        self.edit_mode()
    ...
    ###########################################################################
    #                                                                         #
    # This function will load the data from the csv into the software         #
    # and make it uneditable as it is only for display                        #
    #                                                                         #
    ###########################################################################
    ...
    def load_cd(self):
        ...
        self.ui.line_histology.setText(self.completer_fill(0, clinical_data[5]))
    ...
    # this function gets the data from the csv into a list
    def load_data(self, filename):
        with open(filename, 'rt')as f:
            data = csv.reader(f)
            cd = list(data)
    ...
    # this function converts the code into a full name disease
    def completer_fill(self, type, code):
        if type == 0:  # hist
            result = [i for i in new_hist if i.startswith(code)]
            return result[0]
...
class Transect(QtWidgets.QGraphicsScene):
    ...
    def __init__(self, mainWindow, image_to_paint, dataset, rowS, colS,
                 tabWindow, is_ROI_draw=False):
    ...
    # This function starts the line draw when the mouse is pressed into
    # the 2D view of the scan
    def mousePressEvent(self, event):
        # Clear the current transect first
        plt1.close()
        # If is the first time we can draw as we want a line per button
        # press
    ...
    # This function tracks the mouse and draws the line from the
    ...
    # This function performs the DDA algorithm that locates all the
    # points in the drawn line
    ...
    def plot_result(self):
        ...
        if self.is_ROI_draw:
            ...
            for x in range(len(new_list)):
                if (new_list[x] >= self.thresholds[0] and new_list[x] <=
                        self.thresholds[1]):
...
class MainPageCallClass:
    ...
    def display_cd_dat(self, tabWindow, file_path):
        self.tab_cd = ClinicalDataDisplay(tabWindow, file_path, self.dataset,
                                          self.filepaths)
        tabWindow.addTab(self.tab_cd, "Clinical Data")

    # This function runs Transect on button click
    def runTransect(self, mainWindow, tabWindow, imagetoPaint, dataset, rowS,
                    colS, isROIDraw=False):
